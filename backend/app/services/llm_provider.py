...
class GeminiProvider(LLMProvider):
    """
    Official Google GenAI Python SDK integration (google-genai).
    Uses `from google import genai` and `client = genai.Client(api_key=...)`.
    """
   ...
   async def generate_json(self, prompt: str, schema_description: str) -> Dict[str, Any]:
    ...
    async def generate_text(self, prompt: str) -> str:
        ...
        try:
            ...
            logger.error(f"Gemini generate_text failed ({type(e).__name__}: {str(e)[:200]}). Invoking semantic fallback.")
